...
static CallOutcome classify_outcome(CallCtxEx *ctx, int sip_status)
{
    ...
    if (ctx->early_progress && (sip_status == 408 || sip_status == 487))
        return OUTCOME_NO_ANSWER;
...
static void on_timeout(pj_timer_heap_t *ht, pj_timer_entry *entry)
{
    ...
    if (entry->id == 1) {
        /* Cancel-after-progress timer (500ms) — non-reliable 18x, send CANCEL now */
        if (!ctx->cancel_sent) {
            ctx->cancel_sent = PJ_TRUE;
            LOG(g_tag, ctx->req.request_id, "leg_teardown_initiating", 
                ",\"call_id\":%d,\"trigger\":\"non_reliable_18x_progress_timeout\"", ctx->call_id);
    ...
    switch (ci.state) {
        ...
        case PJSIP_INV_STATE_EARLY:
            if (ci.last_status == 180) {
                ...
                /* Arm short timer (500ms) for reliable PRACK or direct cancel fallback */
                arm_progress_cancel_timer(ctx);
            } else if (ci.last_status == 183) {
                ctx->early_progress = PJ_TRUE;
                LOG(g_tag, ctx->req.request_id, "leg_early_media_183",
                    ",\"call_id\":%d,\"handling\":\"treat_as_early_progress\"", call_id);

                /* 183 may arrive before 180; treat it as valid provisional progress. */
                arm_progress_cancel_timer(ctx);
            } else {
        ...
        case PJSIP_INV_STATE_DISCONNECTED:
            LOG(g_tag, ctx->req.request_id, "leg_state_change",
                ...
                call_id, ci.state, ci.last_status, (int)ci.last_status_text.slen, ci.last_status_text.ptr);
...
static void on_call_tsx_state(pjsua_call_id call_id, pjsip_transaction *tsx, pjsip_event *e)
{
    ...
    /* PRACK 200 OK after early progress → send CANCEL */
    //if (ctx->early_progress && !ctx->cancel_sent &&
        ...
        pj_strcmp2(&tsx->method.name, "PRACK") == 0)
    {
        ...
        pj_timer_heap_cancel(th, &ctx->timer);
        LOG(g_tag, ctx->req.request_id, "leg_teardown_initiating", 
            ",\"call_id\":%d,\"trigger\":\"reliable_prack_ok_after_18x_progress\"", call_id);
        pjsua_call_hangup(call_id, 0, NULL, NULL);
    }
}
...
int sip_engine_init(WorkerConfig *cfg, LFQueue *compq, void (*signal_fn)(void))
{
    ...
    med_cfg.has_ioqueue       = PJ_FALSE; /* disable media ioqueue — no RTP processing needed */
    med_cfg.clock_rate        = 8000;
    med_cfg.audio_frame_ptime = 20;
    med_cfg.max_media_ports   = OBD_CALLS_PER_WORKER * 2 + 4;
    med_cfg.thread_cnt        = 0;        /* no media clock thread — eliminates 50Hz wakeup per worker */
...
void sip_engine_dispatch(const OBDRequest *req)
{
    ...
    /* Assign dedicated non-overlapping RTP port range per worker.
     * Layout: RTP_BASE + (worker_id * slots_per_worker * 2)
     * Each call needs 2 ports (RTP + RTCP), slots = max_calls + headroom */
#define OBD_RTP_SLOTS        (OBD_CALLS_PER_WORKER + 16)  /* calls + headroom */
    acc_cfg.rtp_cfg.port       = g_wcfg.rtp_base_port + (g_wcfg.worker_id * OBD_RTP_SLOTS * 2);
    acc_cfg.rtp_cfg.port_range = (OBD_RTP_SLOTS * 2) - 1;
    ...
    char route_val[256];
    snprintf(route_val, sizeof(route_val), "<sip:%s:%d;transport=udp;lr>", dest, g_wcfg.proxy_port);
