...
void sip_engine_log_health(void)
{
    ...
    LOG(g_tag, "", "worker_health",
        ",\"active_calls\":%d,\"max_calls\":%d,\"max_acc\":%d",
        active, OBD_CALLS_PER_WORKER, PJSUA_MAX_ACC);
}
...
static void log_call_media_stats(CallCtxEx *ctx, const char *phase)
{
    ...
    if (st != PJ_SUCCESS) {
        char errbuf[128];
        LOG(g_tag, ctx->req.request_id, "leg_media_stats_unavailable",
    ...
    if (ci.media_cnt == 0) {
        LOG(g_tag, ctx->req.request_id, "leg_media_rtp_stats",
...
static void on_timeout(pj_timer_heap_t *ht, pj_timer_entry *entry)
{
    ...
    if (entry->id == 2) {
        if (!ctx->done && ctx->call_id != PJSUA_INVALID_ID) {
            LOG(g_tag, ctx->req.request_id, "leg_teardown_executing",
...
static void on_call_state(pjsua_call_id call_id, pjsip_event *e)
{
    ...
    switch (ci.state) {
        ...
        case PJSIP_INV_STATE_DISCONNECTED: {
            ...
            /* Use authoritative final status if already captured by on_call_tsx_state,
             * otherwise fall back to ci.last_status. For CANCEL path (487 expected)
             * synthesize 487 directly — do NOT defer, as the tsx may already be
             * destroyed by the time DISCONNECTED fires after re-entrant hangup. */
...
int sip_engine_init(WorkerConfig *cfg, LFQueue *compq, void (*signal_fn)(void))
{
    ...
    log_cfg.level         = g_wcfg.sip_log_level;
...
LOG(g_tag, "", "proxy_config_check",
    ...
    /* Enable SO_REUSEADDR to allow rapid port reuse after worker restart */
    ...
    for (int attempt = 0; attempt < max_attempts; attempt++) {
        tp_cfg.port = base_port + (attempt * OBD_NUM_WORKERS); /* Skip by worker count to avoid sibling collisions */
        if (cfg->local_ip[0]) {
...
void sip_engine_dispatch(const OBDRequest *req)
{
    ...
    /* RTP ports: each worker gets a non-overlapping block.
     * randomize_port=PJ_TRUE makes PJSIP pick a random start within the
     * range on each call, avoiding TIME_WAIT collisions at high call rates. */
    const unsigned rtp_ports_per_worker = OBD_CALLS_PER_WORKER * 2;
    acc_cfg.rtp_cfg.port           = g_wcfg.rtp_base_port +
                                     (g_wcfg.worker_id * rtp_ports_per_worker);
    acc_cfg.rtp_cfg.port_range     = rtp_ports_per_worker - 1;
    acc_cfg.rtp_cfg.randomize_port = PJ_TRUE;
    ...
    if (atomic_load(&g_active_accs) >= PJSUA_MAX_ACC - 1) {
        ...
        report.sip_final_status = 503;
        lfq_push(g_compq, &report);
        LOG(g_tag, req->request_id, "acc_pool_full",
    ...
    if (st != PJ_SUCCESS) {
        ...
        LOG(g_tag, req->request_id, "leg_invite_failed",
            ...
            st, pj_status_text(st, errbuf, sizeof(errbuf)), atomic_load(&g_active_calls));
