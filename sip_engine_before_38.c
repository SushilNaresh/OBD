...
void sip_engine_dispatch(const OBDRequest *req)
{
    ...
    /* Bind a minimal RTP port for SDP negotiation — required so PJSIP can
     * generate a valid SDP offer/answer. Media direction set to inactive
     * so no RTP packets actually flow. */
    acc_cfg.rtp_cfg.port       = 20000 + (g_wcfg.worker_id * 2);
    ...
    call_opt.aud_cnt = 1;
    call_opt.vid_cnt = 0;
    /* a=inactive in SDP: carrier gets valid SDP offer/answer, no RTP flows */
