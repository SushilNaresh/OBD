...
tune:
	$(CC) $(CFLAGS) -g \
		...
		$(if $(OBD_NUM_LOCAL_IPS),-DOBD_NUM_LOCAL_IPS=$(OBD_NUM_LOCAL_IPS)) \
		...
		$(if $(OBD_ROUTING_THREADS),-DOBD_ROUTING_THREADS=$(OBD_ROUTING_THREADS)) \
		$(if $(OBD_PKTQ_SIZE),-DOBD_PKTQ_SIZE=$(OBD_PKTQ_SIZE)) \
