...
static int startup_preflight(const WorkerConfig *worker_cfgs)
{
    ...
    if (preflight_udp_bind("", g_listen_port, reason, sizeof(reason)) != 0) {
        LOG("main", "", "startup_port_conflict",
...
int main(int argc, char *argv[])
{
    ...
    if (argc >= 3 && strcmp(argv[1], "--config") == 0) {
        ...
        if (argc > 3) strncpy(g_local_ip, argv[3], sizeof(g_local_ip) - 1);
    ...
    LOG("main", "", "starting",
        ",\"workers\":%d,\"total_capacity\":%d,\"listen_port\":%d,\"heartbeat_port\":%d,\"sip_port_base\":%d,\"mux_port\":%d,\"proxy_port\":%d,\"rtp_base_port\":%d,\"use_mux\":\"%s\",\"log_file\":\"%s\"",
    ...
    dispatcher_run(g_local_ip, g_listen_port, g_report_ip, g_report_port,
